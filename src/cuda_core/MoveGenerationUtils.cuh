...
HYBRID [[nodiscard]] constexpr cuda_Array<uint64_t, BIT_BOARD_FIELDS>
GenStaticMoves(const int maxMovesCount, const int *movesCords, const int *rowCords) {
    cuda_Array<uint64_t, BIT_BOARD_FIELDS> movesRet{};
...
HYBRID [[nodiscard]] constexpr uint32_t
...
HYBRID [[nodiscard]] constexpr uint64_t
...
GenMask(const int startInd, const int boarderIndex, const int offset) {
    uint64_t ret = 0;
    for (uint32_t i = startInd; i < boarderIndex; i += offset) ret |= (1LLU << i);
    return ret;
}

HYBRID [[nodiscard]] constexpr uint32_t MyCeil(const double x) {
    return (static_cast<double>(static_cast<uint32_t>(x)) == x) ? static_cast<uint32_t>(x)
                                                                  : static_cast<uint32_t>(x) + ((x > 0) ? 1 : 0);
}


template<class BoundaryCheckFuncT>
HYBRID constexpr uint64_t
GenSlidingMoves(const uint64_t neighbors, const int bInd, const int offset, const int boundary,
...
MoveInitializer(MapT &map, MoveGeneratorT mGen, NeighborGeneratorT nGen, NeighborStripT nStrip, const int bInd) {
    ...
    for (uint32_t j = 0; j < posSize; ++j) {
        const uint64_t strippedNeighbors = nStrip(possibilities[j], map.getMasks());
