...
static constexpr __uint32_t MINIMAL_SPLIT_BATCH_SIZE = WARP_SIZE * BIT_BOARDS_PER_COLOR;
static constexpr __uint32_t SPLIT_BATCH_NUM_WARPS = 2;
static constexpr __uint32_t SPLIT_BATCH_THREADS = MINIMAL_SPLIT_BATCH_SIZE * SPLIT_BATCH_NUM_WARPS;
static constexpr __uint32_t SINGLE_BATCH_BOARD_SIZE = WARP_SIZE * SPLIT_BATCH_NUM_WARPS;
...
FAST_CALL_ALWAYS __uint32_t CalcBoardIdx(const __uint32_t tx, const __uint32_t bx, const __uint32_t bs) {
    ...
    return (WARP_SIZE * (plainIdx / MINIMAL_SPLIT_BATCH_SIZE)) + (tx % WARP_SIZE);
}
...
FAST_CALL_ALWAYS __uint32_t CalcResourceIdx(const __uint32_t tx) {
    return WARP_SIZE * (tx / MINIMAL_SPLIT_BATCH_SIZE) + (tx % WARP_SIZE);
}
